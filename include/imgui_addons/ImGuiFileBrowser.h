...
namespace imgui_addons
{
    class ImGuiFileBrowser
    {
        public:
            ...
            std::string current_path_override;
            std::string ext;    // Store the saved file extension

            // Keeps track of whether file dialogue was just cancelled
            bool IsCancelJustPressed(const std::string &label);


        private:
            std::map<std::string, bool> cancelPressedDirtyFlags;
            struct Info
            {
            ...
            bool renderButtonsAndCheckboxRegion(const std::string& label);
