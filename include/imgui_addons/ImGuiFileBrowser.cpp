...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types)
    {
        ...
        if (cancelPressedDirtyFlags.size(label) > 0 && cancelPressedDirtyFlags[label]) {
            cancelPressedDirtyFlags[label] = false;
        }
        ...
        if (ImGui::BeginPopupModal(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            show_error |= renderButtonsAndCheckboxRegion(label);
    ...
    bool ImGuiFileBrowser::IsCancelJustPressed(const std::string &label) {
        if (cancelPressedDirtyFlags.size(label) > 0 && cancelPressedDirtyFlags[label]) {
            return true;
        }
        return false;
    ...
    bool ImGuiFileBrowser::renderFileListRegion()
    {
        ...
        for (int i = 0; i < filtered_dirs.size(); i++)
        {
            ...
            {
                items++;
                if(ImGui::Selectable(filtered_dirs[i]->gameTitle.c_str(), selected_idx == i && is_dir, ImGuiSelectableFlags_AllowDoubleClick))
                {
                    selected_idx = i;
                    is_dir = true;
                    ...
                    if(dialog_mode == DialogMode::SELECT)
                        strcpy(input_fn, filtered_dirs[i]->gameTitle.c_str());
        ...
        for (int i = 0; i < filtered_files.size(); i++)
        {
            ...
            {
                items++;
                if(ImGui::Selectable(filtered_files[i]->gameTitle.c_str(), selected_idx == i && !is_dir, ImGuiSelectableFlags_AllowDoubleClick))
                {
                    ...
                    strcpy(input_fn, filtered_files[i]->gameTitle.c_str());

                    if(ImGui::IsMouseDoubleClicked(0))
                    {
                        selected_fn = filtered_files[i]->gameTitle;
                        validate_file = true;
                    }
                }
                if( (items) % col_items_limit == 0)
    ...
    bool ImGuiFileBrowser::renderInputTextAndExtRegion()
    {
        ...
        if(ImGui::IsItemEdited() || ImGui::IsItemActivated())
        {
            ...
            if(dialog_mode == DialogMode::OPEN || dialog_mode == DialogMode::SAVE)
            {
                ...
                {
                    if(ImStristr(subfiles[i].gameTitle.c_str(), nullptr, input_fn, nullptr) != nullptr)
                        inputcb_filter_files.push_back(std::ref(subfiles[i].gameTitle));
                }
            }
            ...
            else if(dialog_mode == DialogMode::SELECT)
            {
                ...
                {
                    if(ImStristr(subdirs[i].gameTitle.c_str(), nullptr, input_fn, nullptr) != nullptr)
                        inputcb_filter_files.push_back(std::ref(subdirs[i].gameTitle));
                }
            }
    ...
    bool ImGuiFileBrowser::renderButtonsAndCheckboxRegion(const std::string& label)
    {
        ImVec2 pw_size = ImGui::GetWindowSize();
        ImGuiStyle& style = ImGui::GetStyle();
        bool show_error = false;
        ...
        if (ImGui::Button("Cancel")) {
            if (cancelPressedDirtyFlags.size(label) <= 0) {
                cancelPressedDirtyFlags.emplace(label, true);
            }
            cancelPressedDirtyFlags[label] = true;
            closeDialog();
        }
    ...
    bool ImGuiFileBrowser::onDirClick(int idx)
    {
        ...
        name = filtered_dirs[idx]->gameTitle;
    ...
    void ImGuiFileBrowser::filterFiles(int filter_mode)
    {
        ...
        if(filter_mode | FilterMode_Dirs)
        {
            ...
            {
                if(filter.PassFilter(subdirs[i].gameTitle.c_str()))
        ...
        if(filter_mode | FilterMode_Files)
        {
            ...
            for (size_t i = 0; i < subfiles.size(); ++i)
            {
                ...
                {
                    if(filter.PassFilter(subfiles[i].gameTitle.c_str()))
                        filtered_files.push_back(&subfiles[i]);
                }
                else
                {
                    if(filter.PassFilter(subfiles[i].gameTitle.c_str()) && (ImStristr(subfiles[i].gameTitle.c_str(), nullptr, valid_exts[selected_ext_idx].c_str(), nullptr)) != nullptr)
                        filtered_files.push_back(&subfiles[i]);
                }
            }
        }
    ...
    bool ImGuiFileBrowser::validateFile()
    {
        ...
        {
            if(dialog_mode == DialogMode::SELECT)
                match = (filtered_dirs[selected_idx]->gameTitle == selected_fn);
            else
                match = (filtered_files[selected_idx]->gameTitle == selected_fn);
        }
        ...
        if(!match)
        {
            if(dialog_mode == DialogMode::SELECT)
            {
                for(int i = 0; i < subdirs.size(); i++)
                {
                    if(subdirs[i].gameTitle == selected_fn)
                    {
                        match = true;
                        break;
                    }
            ...
            else
            {
                for(int i = 0; i < subfiles.size(); i++)
                {
                    if(subfiles[i].gameTitle == selected_fn)
                    {
                        match = true;
                        break;
                    }
    ...
    void ImGuiFileBrowser::initCurrentPath()
    {
        ...
        {
            bool isCurrentPathOverrideValid = !current_path_override.empty(); // TODO: better path validation
            if (!isCurrentPathOverrideValid)
            {
                current_path = "/";
                current_dirlist.push_back("/");
            }
            else
            {
                ...
                current_dirlist.push_back(current_path_override);
            }
        }
        else
        {
