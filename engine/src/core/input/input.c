...
void rbe_input_add_action_value(const char* actionName, const char* actionValue, int deviceId) {
    if (!rbe_string_hash_map_has(inputActionMap, actionName)) {
        rbe_string_hash_map_add(inputActionMap, actionName,
                                rbe_input_action_create_new_input_action(actionName, deviceId), sizeof(InputAction));
    ...
    if (rbe_string_hash_map_has(keyboardStringValuesMap, actionValue)) {
        inputAction->keyboardValues[inputAction->keyboardValueCount] =
            rbe_string_hash_map_get_int(keyboardStringValuesMap, actionValue);
        inputAction->keyboardValueCount++;
        rbe_logger_debug("Added keyboard action | name: '%s', value: '%s', scancode: '%d'", actionName, actionValue,
    ...
    } else if (strcmp(actionValue, "mb_left") == 0 || strcmp(actionValue, "mb_right") == 0 ||
               strcmp(actionValue, "mb_middle") == 0) {
        strcpy(&inputAction->mouseValues[inputAction->mouseValueCount], actionValue);
        rbe_logger_debug("Added mouse action | name: '%s', value: '%s'", actionName,
    ...
    for (size_t i = 0; i < inputActionNamesCount; i++) {
        ...
        for (size_t j = 0; j < inputAction->keyboardValueCount; j++) {
            ...
            if (isKeyboardValuePressed && !inputAction->isActionPressed) {
                //                rbe_logger_debug("isKeyboardValuePressed = %d, scancode = %d", isKeyboardValuePressed,
                //                scancode);
                ...
                break;
            } else if (!isKeyboardValuePressed && !inputAction->isActionJustReleased && inputAction->isActionPressed &&
                       (int) scancode == inputAction->lastScancodePressed) {
        ...
        for (size_t i = 0; i < inputActionNamesCount; i++) {
            ...
            for (size_t j = 0; j < inputAction->mouseValueCount; j++) {
                bool isLeftMouseButton =
                    strcmp(&inputAction->mouseValues[j], "mb_left") == 0 && mouseButton == SDL_BUTTON_LEFT;
                bool isRightMouseButton =
                    strcmp(&inputAction->mouseValues[j], "mb_right") == 0 && mouseButton == SDL_BUTTON_RIGHT;
...
void input_initialize_gamepad_system() {
    ...
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_right_shoulder",
                                GamepadInputButtonType_RIGHT_SHOULDER);
    ...
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_left_analog_left",
                                GamepadInputButtonType_LEFT_ANALOG_LEFT);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_left_analog_right",
                                GamepadInputButtonType_LEFT_ANALOG_RIGHT);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_left_analog_up",
                                GamepadInputButtonType_LEFT_ANALOG_UP);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_left_analog_down",
                                GamepadInputButtonType_LEFT_ANALOG_DOWN);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_right_analog_left",
                                GamepadInputButtonType_RIGHT_ANALOG_LEFT);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_right_analog_right",
                                GamepadInputButtonType_RIGHT_ANALOG_RIGHT);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_right_analog_up",
                                GamepadInputButtonType_RIGHT_ANALOG_UP);
    rbe_string_hash_map_add_int(gamepadStringValuesMap, "joystick_right_analog_down",
...
bool input_process_axis_motions() {
    ...
    // LEFT AXIS
    // Horizontal
    int16_t leftHorizontalValue =
        SDL_JoystickGetAxis(joystickController, GamepadInputAxisMotionType_LEFT_HORIZONTAL_AXIS);
    ...
    // RIGHT AXIS
    // Horizontal
    int16_t rightHorizontalValue =
        SDL_JoystickGetAxis(joystickController, GamepadInputAxisMotionType_RIGHT_HORIZONTAL_AXIS);
    ...
    // Vertical
    int16_t rightVerticalValue =
        SDL_JoystickGetAxis(joystickController, GamepadInputAxisMotionType_RIGHT_VERTICAL_AXIS);
