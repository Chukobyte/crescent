...
void animated_sprite_rendering_system_render() {
    ...
    for (size_t i = 0; i < animatedSpriteRenderingSystem->entity_count; i++) {
        ...
        const Transform2DComponent* spriteTransformComp =
            (Transform2DComponent*) component_manager_get_component(entity, ComponentDataIndex_TRANSFORM_2D);
        AnimatedSpriteComponent* animatedSpriteComponent =
            (AnimatedSpriteComponent*) component_manager_get_component(entity, ComponentDataIndex_ANIMATED_SPRITE);
        AnimationFrame currentFrame = animatedSpriteComponent->currentAnimation
                                          .animationFrames[animatedSpriteComponent->currentAnimation.currentFrame];
        if (animatedSpriteComponent->isPlaying) {
            const int newIndex = ((currentTickTime - (int) animatedSpriteComponent->startAnimationTickTime) /
                                  animatedSpriteComponent->currentAnimation.speed) %
                                 animatedSpriteComponent->currentAnimation.frameCount;
        ...
        const Rect2 destinationRectangle = {
            ...
            currentFrame.drawSource.w * spriteTransformComp->scale.x * parentTransform.scale.x,
            currentFrame.drawSource.h * spriteTransformComp->scale.y * parentTransform.scale.y};
        rbe_renderer_queue_sprite_draw_call(currentFrame.texture, currentFrame.drawSource, destinationRectangle,
                                            spriteTransformComp->rotation, animatedSpriteComponent->modulate,
                                            animatedSpriteComponent->flipX, animatedSpriteComponent->flipY);
    }
}
