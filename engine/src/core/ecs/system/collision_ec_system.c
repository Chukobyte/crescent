...
EntitySystem* collision_ec_system_get() { return collisionSystem; }
...
void collision_system_render() {
    for (size_t i = 0; i < collisionSystem->entity_count; i++) {
        ...
        const Collider2DComponent* colliderComp =
            (Collider2DComponent*) component_manager_get_component(entity, ComponentDataIndex_COLLIDER_2D);
        ...
        rbe_renderer_queue_sprite_draw_call(collisionOutlineTexture, colliderDrawSource, colliderDrawDestination,
                                            transformComp->rotation, colliderComp->color, false, false);
    }
}
