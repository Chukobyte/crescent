...
void script_system_on_entity_registered(Entity entity) {
    const ScriptComponent* scriptComponent =
        (ScriptComponent*) component_manager_get_component(entity, ComponentDataIndex_SCRIPT);
    ...
    scriptContexts[scriptComponent->contextType]->on_create_instance(entity, scriptComponent->classPath,
                                                                     scriptComponent->className);
}
...
void script_system_entity_start(Entity entity) {
    ...
    RBE_ASSERT_FMT(scriptComponent->contextType == ScriptContextType_PYTHON ||
