...
void cre_camera2d_clamp_viewport_to_boundary(CRECamera2D* camera2D) {
    ...
    } else if (camera2D->viewport.x > camera2D->boundary.w - (f32) gameProperties->resolutionWidth) {
        camera2D->viewport.x = camera2D->boundary.w - (f32) gameProperties->resolutionWidth;
    }
