...
Texture* rbe_asset_manager_load_texture(const char* fileName, const char* key) {
    ...
    RBE_ASSERT_FMT(!rbe_string_hash_map_has(texturesMap, fileName),
                   "Already loaded texture at file path '%'s!  Has key '%s'.", fileName, key);
    Texture* texture = rbe_texture_create_texture(fileName);
    rbe_string_hash_map_add(texturesMap, key, texture, sizeof(Texture));
    return texture;
}

Texture* rbe_asset_manager_get_texture(const char* key) { return (Texture*) rbe_string_hash_map_get(texturesMap, key); }

bool rbe_asset_manager_has_texture(const char* key) { return rbe_string_hash_map_has(texturesMap, key); }
...
Font* rbe_asset_manager_get_font(const char* key) { return (Font*) rbe_string_hash_map_get(fontMap, key); }

bool rbe_asset_manager_has_font(const char* key) { return rbe_string_hash_map_has(fontMap, key); }

// --- Audio Source --- //
RBEAudioSource* rbe_asset_manager_load_audio_source_wav(const char* fileName, const char* key) {
    RBE_ASSERT(audioSourceMap != NULL);
    RBE_ASSERT_FMT(!rbe_string_hash_map_has(audioSourceMap, fileName),
                   "Already loaded audio source at file path '%'s!  Has key '%s'.", fileName, key);
    int32_t sampleCount;
    int32_t channels;
    int32_t sampleRate;
    void* samples = NULL;
    ...
    RBEAudioSource* newAudioSource =
        (RBEAudioSource*) RBE_MEM_ALLOCATE_SIZE(sizeof(RBEAudioSource*) + (sampleCount * sizeof(int16_t*)));
...
bool rbe_asset_manager_has_audio_source(const char* key) { return rbe_string_hash_map_has(audioSourceMap, key); }
