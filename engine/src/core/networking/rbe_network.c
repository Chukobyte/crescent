...
bool rbe_network_is_server() { return rbe_is_server; }
...
void* rbe_udp_server_poll(void* arg) {
    ...
    while (true) {
        ...
        if ((server_recv_len = recvfrom(server_socket, server_input_buffer, SERVER_BUFFER_SIZE, 0,
                                        (struct sockaddr*) &server_si_other, &server_socket_size)) == SOCKET_ERROR) {
        ...
        //        rbe_logger_debug("Received packet from %s:%d", inet_ntoa(server_si_other.sin_addr),
        //        ntohs(server_si_other.sin_port)); rbe_logger_debug("Data: %s", server_input_buffer);
    }
    return NULL;
}
...
bool rbe_udp_server_send_message(const char* message) {
    ...
    if (sendto(server_socket, server_output_buffer, server_recv_len, 0, (struct sockaddr*) &server_si_other,
               server_socket_size) == SOCKET_ERROR) {
...
void* rbe_udp_client_poll(void* unused) {
    ...
    while (true) {
        ...
        if (recvfrom(client_socket, client_input_buffer, CLIENT_BUFFER_SIZE, 0, (struct sockaddr*) &client_si_other,
                     &client_socket_size) == SOCKET_ERROR) {
...
bool rbe_udp_client_send_message(const char* message) {
    ...
    if (sendto(client_socket, client_output_buffer, (int) strlen(client_output_buffer), 0,
               (struct sockaddr*) &client_si_other, client_socket_size) == SOCKET_ERROR) {
