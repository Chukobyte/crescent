...
PyObject* rbe_py_api_configure_game(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "siiiiisb", rbePyApiProjectConfigureKWList, &gameTitle, &windowWidth,
                                    &windowHeight, &resolutionWidth, &resolutionHeight, &targetFPS, &initialScenePath,
                                    &collidersVisible)) {
...
PyObject* rbe_py_api_configure_assets(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    PyObject* fontsList;
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "OOO", rbePyApiConfigureAssetsKWList, &audioSourcesList,
                                    &texturesList, &fontsList)) {
        RBE_ASSERT_FMT(PyList_Check(audioSourcesList),
                       "Passed in audio source assets are not a python list, check python api implementation...");
        RBE_ASSERT_FMT(PyList_Check(texturesList),
                       "Passed in texture assets are not a python list, check python api implementation...");
        RBE_ASSERT_FMT(PyList_Check(fontsList),
                       "Passed in font assets are not a python list, check python api implementation...");
        ...
        for (Py_ssize_t i = 0; i < PyList_Size(texturesList); i++) {
            ...
            rbe_logger_debug("file_path = %s, wrap_s = %s, wrap_t = %s, filter_min = %s, filter_mag = %s", filePath,
...
PyObject* rbe_py_api_configure_inputs(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "O", rbePyApiConfigureInputsKWList, &inputActionsList)) {
        RBE_ASSERT_FMT(PyList_Check(inputActionsList),
                       "Passed in input actions are not a python list, check python api implementation...");
        ...
        for (Py_ssize_t i = 0; i < PyList_Size(inputActionsList); i++) {
            ...
            RBE_ASSERT_FMT(PyList_Check(valuesList),
                           "Input action values for '%s' is not a list!  Check python api implementation.", actionName);
            Py_ssize_t valueListSize   = PyList_Size(valuesList);
            RBEInputAction inputAction = {
                .name = rbe_strdup(actionName), .deviceId = actionDeviceId, .valueCount = (size_t) valueListSize};
...
PyObject* rbe_py_api_create_stage_nodes(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "O", rbePyApiCreateStageNodesKWList, &stageNodeList)) {
        RBE_ASSERT_FMT(PyList_Check(stageNodeList),
                       "Passed in stage nodes are not a python list, check python api implementation...");
...
PyObject* PyInit_rbe_py_API(void) { return PyModule_Create(&rbePyAPIModDef); }
...
void setup_scene_stage_nodes(SceneTreeNode* parent, PyObject* stageNodeList) {
    for (Py_ssize_t i = 0; i < PyList_Size(stageNodeList); i++) {
        ...
        RBE_ASSERT_FMT(nodeComponent->type != NodeBaseType_INVALID, "Node '%s' has an invalid node type '%s'", nodeName,
                       nodeType);
...
void setup_scene_component_node(Entity entity, PyObject* component) {
    ...
    if (strcmp(className, "Transform2DComponent") == 0) {
        ...
        rbe_logger_debug(
            "position: (%f, %f), scale: (%f, %f), rotation: %f, z_index: %d, z_index_relative: %d, ignore_camera: %d",
        ...
        rbe_logger_debug(
            "texture_path = %s, draw_source = (%f, %f, %f, %f), flip_x: %d, flip_y: %d, modulate: (%d, %d, %d, %d)",
            texturePath, drawSourceX, drawSourceY, drawSourceW, drawSourceH, flipX, flipY, modulateR, modulateG,
            modulateB, modulateA);
        ...
        const bool flipX                                 = phy_get_bool_from_var(component, "flip_x");
        const bool flipY                                 = phy_get_bool_from_var(component, "flip_y");
        rbe_logger_debug("current_animation_name: '%s', is_playing: '%d', flip_x: '%d', flip_y: '%d'",
                         currentAnimationName, isPlaying, flipX, flipY);
        ...
        for (Py_ssize_t animationIndex = 0; animationIndex < PyList_Size(pyAnimationsList); animationIndex++) {
            ...
            rbe_logger_debug("building anim - name: '%s', speed: '%d', loops: '%d'", animationName, animationSpeed,
                             animationLoops);
            ...
            for (Py_ssize_t animationFrameIndex = 0; animationFrameIndex < PyList_Size(pyAnimationFramesList);
                 animationFrameIndex++) {
                ...
                rbe_logger_debug("frame: %d, texture_path: %s, draw_source: (%f, %f, %f, %f)", animationFrameNumber,
                                 animationFrameTexturePath, drawSourceX, drawSourceY, drawSourceW, drawSourceH);
        ...
        rbe_logger_debug("uid: %s, text: %s, color(%d, %d, %d, %d)", textLabelUID, textLabelText, colorR, colorG,
                         colorB, colorA);
        Py_DECREF(pColor);
        ...
        rbe_logger_debug("rect: (%f, %f, %f, %f), color: (%f, %f, %f, %f)", rectX, rectY, rectW, rectH,
...
PyObject* rbe_py_api_input_add_action(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    char* actionValue;
    int deviceId;
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "ssi", rbePyApiInputAddActionKWList, &actionName, &actionValue,
                                    &deviceId)) {
...
#define TYPE_BUFFER_SIZE 32
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "is", rbePyApiNodeGetChildKWList, &parentEntity, &childName)) {
        ...
        if (childEntity == NULL_ENTITY) {
            rbe_logger_warn("Failed to get child node from parent entity '%d' with the name '%s'", parentEntity,
                            childName);
            Py_RETURN_NONE;
        }
...
PyObject* rbe_py_api_node2D_get_position(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "i", rbePyApiGenericGetEntityKWList, &entity)) {
        const Transform2DComponent* transformComp =
            (Transform2DComponent*) component_manager_get_component(entity, ComponentDataIndex_TRANSFORM_2D);
...
PyObject* rbe_py_api_text_label_set_color(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    int blue;
    int alpha;
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "iffff", rbePyApiTextLabelSetColorKWList, &entity, &red, &green,
                                    &blue, &alpha)) {
        TextLabelComponent* textLabelComponent = component_manager_get_component(entity, ComponentDataIndex_TEXT_LABEL);
...
PyObject* rbe_py_api_server_subscribe(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "siO", rbePyApiNetworkSubscribeKWList, &signalId, &listenerNode,
                                    &listenerFunc)) {
