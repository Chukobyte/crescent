...
RBEScriptContext* rbe_py_get_script_context() { return python_script_context; }
...
void py_on_delete_instance(Entity entity) {
    ...
    if (current_network_server_client_connected_script_callback != NULL &&
        entity == current_network_server_client_connected_script_callback->entity) {
...
void py_on_start(Entity entity) {
    RBE_ASSERT_FMT(rbe_hash_map_has(pythonInstanceHashMap, &entity),
                   "Tried to call py on_start to non existent python instance entity '%d'", entity);
    PyObject* pScriptInstance = (PyObject*) *(PyObject**) rbe_hash_map_get(pythonInstanceHashMap, &entity);
...
void py_on_entity_subscribe_to_network_callback(Entity entity, PyObject* callback_func, const char* id) {
    ...
    if (strcmp(id, "poll") == 0) {
        ...
        if (current_network_server_client_connected_script_callback == NULL) {
            ...
            Py_IncRef(current_network_server_client_connected_script_callback
                          ->callback_func); // Increase ref to hold on to function
