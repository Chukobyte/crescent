...
void cre_pkpy_api_load_internal_modules() {
    ...
    cre_pkpy_util_create_module(&(CrePPModule) {
        ...
        .functions = {
            ...
            {.signature = "animated_sprite_add_animation(entity_id: int, name: str, speed: float, does_loop: bool, frames: List[\"AnimationFrame\"]) -> None", .function = cre_pkpy_api_animated_sprite_add_animation},
