...
Rect2 get_collision_rectangle(Transform2DComponent* transform2DComponent, Collider2DComponent* collider2DComponent);
...
CollisionResult rbe_collision_process_entity_collisions(Entity entity) {
    ...
    Rect2 sourceCollisionRect = get_collision_rectangle(transformComponent, colliderComponent);
    for (size_t i = 0; i < collisionSystem->entity_count; i++) {
        ...
        Rect2 otherCollisionRect = get_collision_rectangle(otherTransformComponent, otherColliderComponent);
...
Rect2 get_collision_rectangle(Transform2DComponent* transform2DComponent, Collider2DComponent* collider2DComponent) {
    Rect2 collisionRect = {
        .x = transform2DComponent->position.x + collider2DComponent->rect.x,
        .y = transform2DComponent->position.y + collider2DComponent->rect.y,
        .w = transform2DComponent->scale.x * collider2DComponent->rect.w,
        .h = transform2DComponent->scale.y * collider2DComponent->rect.h,
    };
