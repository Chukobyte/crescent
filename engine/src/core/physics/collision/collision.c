...
Rect2 get_collision_rectangle(Entity entity, Transform2DComponent* transform2DComponent,
...
CollisionResult rbe_collision_process_entity_collisions(Entity entity) {
    ...
    for (size_t i = 0; i < collisionSystem->entity_count; i++) {
        ...
        Transform2DComponent* otherTransformComponent =
            component_manager_get_component(entity, ComponentDataIndex_TRANSFORM_2D);
        Collider2DComponent* otherColliderComponent =
            component_manager_get_component(entity, ComponentDataIndex_COLLIDER_2D);
        Rect2 otherCollisionRect =
            get_collision_rectangle(otherEntity, otherTransformComponent, otherColliderComponent);
...
bool does_rectangles_collide(Rect2* sourceRect, Rect2* targetRect) {
    return (sourceRect->x + sourceRect->w >= targetRect->x) && (targetRect->x + targetRect->w >= sourceRect->x) &&
           (sourceRect->y + sourceRect->h >= targetRect->y) && (targetRect->y + targetRect->h >= sourceRect->y);
}
