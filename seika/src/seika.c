...
bool initialize_audio(uint32_t waveSampleRate);
...
bool sf_initialize_simple(const char* title, int windowWidth, int windowHeight) {
    return sf_initialize(title, windowWidth, windowHeight, windowWidth, windowHeight, SE_AUDIO_SOURCE_DEFAULT_WAVE_SAMPLE_RATE, false, NULL);
}
...
bool sf_initialize(const char* title,
    ...
    if (!initialize_audio(audioWaveSampleRate)) {
...
bool initialize_audio(uint32_t waveSampleRate) {
    return se_audio_manager_init(waveSampleRate);
}
